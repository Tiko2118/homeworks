age1 =10
age2 =5
if age1 > age2:
    print("1 > 2")
elif age2 > age1:
    print("2 > 1")




my_name = "tiko"

#საიტერაციო ცვლადი
for char in my_name:
    print(char)


print("i" in "tiko")
     
my_name = "salome"
for char in my_name:
    print(char)     

my_name = "salome", "natia"
for char in my_name:
    print(char)    

 #მომხმარებელს შემოატანინეთ ორი სახელი და ის დამიპრინტეთ 
 # რომლის სახელშიც მეტი ხმოვანი იქნება (დააფორმატეთ პასუხი ლამაზად)

name1 = input("enter name1: ")
name2 = input("enter name2: ")

#შევქმნათ ორი საინკრემენტაციო ცვლადი
ammount_of_vowels_in_name1 = 0
ammount_of_vowels_in_name2 = 0

#nika
for char in name1:
    if char in "aeiou":
        ammount_of_vowels_in_name1 += 1
for char in name2:
    if char in "aeiou":
        ammount_of_vowels_in_name2 += 1

if ammount_of_vowels_in_name1 >ammount_of_vowels_in_name2:
    print("the ammount of vowels in name 1 is more and it contains {} vowels".format(ammount_of_vowels_in_name1))
elif ammount_of_vowels_in_name1 <ammount_of_vowels_in_name2:
    print("the ammount of vowels in name 2 is more and it contains {} vowels".format(ammount_of_vowels_in_name2))
else: print ("they have equal ammount of vowels")    


      #შემოიტანეთ რაღაც წინადადება და დაითვალეთ რამდენი "ა" გვხვდება

my_txt = input("enter some txt: ")
ammount_of_a = 0

for char in my_txt:
   if char == "a":
    ammount_of_a += 1
print('there is {} "a" in my text'. format(ammount_of_a))








spam = 7
if spam > 5:
    print("five")
elif spam > 3:
    print("three")   
else:
    print("zzz")

    num = 12
    if num > 5:
        print("Bigger than 5")
        if num <=47:
            print("Between 5 and 47")
                 

